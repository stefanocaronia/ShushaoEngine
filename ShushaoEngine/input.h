...
namespace ShushaoEngine {
	...
	struct ControllerDevice {
		...
		std::string toString() {
			if (controller != nullptr) return "Controller (" + ts(index) + ") [" + ts(instanceID) + "] " + SDL_GameControllerName(controller);
			else if (joystick != nullptr) return "Joystick (" + ts(index) + ") [" + ts(instanceID) + "] " + SDL_JoystickName(joystick);
			return "";
		}
	};
